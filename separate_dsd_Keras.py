...
def stft_norm(data, window=sinebell(2048),
    ...
    return STFT.T #tf.convert_to_tensor
...
def build_ca(input_var=None, batch_size=32,time_context=30,feat_size=513):
    ...
    ### MODIFICAR A KERAS, igual que en trainCNN_Keras  ###
...
def train_auto(filein,outdir,model,scale_factor=0.3,time_context = 30,overlap = 20,batch_size=32,input_size=513):
    ...
    # Pasar a Tensorflow placeholders. Después hay que usar feed_dict cuando entrenamos... Ahí se interpreta el shape de cada uno (4-D)
    input_var2 = tf.placeholder(name='inputs', shape = [None, None, None, None]) 
    target_var2 = tf.placeholder(name='targets', shape = [None, None, None, None]) 
    rand_num = tf.placeholder(name='rand_num', shape = [None, None, None, None])
    ...
    prediction2 = lasagne.layers.get_output(network2, deterministic=True) #Obtener salida de la última capa del modelo (model.get_output)
